...
export const BulkFeeVoucher = ({ students, onClose }: BulkFeeVoucherProps) => {
    ...
    return (
        <div className="bulk-voucher-studio fixed inset-0 z-[1000] bg-[#020617] flex flex-col font-serif print:relative print:z-0 print:bg-white print:block print:inset-auto">
            ...
            <div className="p-3 sm:p-4 border-b border-white/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2.5 sm:gap-4 bg-slate-900/90 backdrop-blur-md print:hidden shrink-0">
                <div className="flex items-center justify-between w-full sm:w-auto gap-3">
                    <div className="flex items-center gap-2.5 sm:gap-3">
                        <div className="w-9 h-9 sm:w-12 sm:h-12 bg-brand-primary rounded-xl sm:rounded-[var(--brand-radius,1rem)] flex items-center justify-center text-brand-accent shadow-2xl border border-white/10 shrink-0">
                            <Layers className="w-5 h-5 sm:w-7 sm:h-7" />
                        </div>
                        <div>
                            <h2 className="text-base sm:text-2xl font-black text-white uppercase tracking-tighter leading-none">Bulk Voucher Studio</h2>
                            <p className="text-[9px] sm:text-[11px] font-black text-slate-400 uppercase tracking-widest mt-1 flex items-center gap-1.5">
                                <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span>
                                {students.length} Accounts Ready
                            </p>
                        </div>
                    </div>
                    <button onClick={onClose} className="sm:hidden p-2 bg-white/5 text-white hover:bg-rose-500/20 hover:text-rose-500 rounded-xl transition-all border border-white/10 shrink-0">
                        <X className="w-4 h-4" />
                    </button>
                </div>
                <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto justify-end">
                    <button
                        ...
                        className={`flex-1 sm:flex-none px-3.5 sm:px-8 py-2.5 sm:py-4 bg-brand-primary text-white rounded-xl sm:rounded-[var(--brand-radius,1rem)] font-black text-[10px] sm:text-[12px] uppercase tracking-widest flex items-center justify-center gap-2 transform transition-all active:scale-95 shadow-2xl shadow-brand-primary/20 group ${isExporting ? 'opacity-50 cursor-not-allowed' : 'hover:opacity-90'}`}
                    >
                    ...
                    <button onClick={onClose} className="hidden sm:block p-2.5 sm:p-4 bg-white/5 text-white hover:bg-rose-500/20 hover:text-rose-500 rounded-[var(--brand-radius,1rem)] transition-all border border-white/10 shrink-0">
            ...
            {/* Mobile Swipe Hint */}
            <div className="md:hidden bg-blue-950/80 px-3 py-1.5 border-b border-blue-500/20 text-center shrink-0">
                <span className="text-[9px] font-bold text-blue-300 uppercase tracking-widest">
                    Swipe horizontally ↔ to view complete 3-copy slip
                </span>
            </div>
