...
export const TeacherIDCardGenerator = ({ teacher, onClose }: TeacherIDCardGeneratorProps) => {
    ...
    const exportHighQualImage = async (side: 'front' | 'back') => {
        ...
        try {
            ...
            const fileName = `${teacher.name.replace(/\s+/g, '_')}_ID_${side.toUpperCase()}.png`;
            await saveAndShareBase64(dataUrl, fileName, `Teacher ID Card (${side.toUpperCase()})`);
        } catch (error) {
    ...
    const exportPDF = async () => {
        ...
        try {
            ...
            const fileName = `${teacher.name.replace(/\s+/g, '_')}_ID_Card.pdf`;
            await saveAndSharePDF(pdf, fileName, `Teacher ID Card - ${teacher.name}`);
                ...
                <div className="p-3 sm:p-4 border-b border-slate-100 flex items-center justify-between gap-2 bg-slate-50/90 shrink-0">
                    <div className="flex items-center gap-2 sm:gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                            <h2 className="text-sm sm:text-lg font-black text-[#003366] uppercase tracking-tight truncate leading-none">Faculty Identity Studio</h2>
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5 truncate">{teacher.name}</p>
                        </div>
                    </div>
                            ...
                            >
                                ...
                                <div className="flex-1 flex flex-col items-center px-5 relative z-10 -mt-2.5">
                                        ...
                                        <div className="w-[28mm] h-[28mm] bg-white rounded-full border-[1mm] border-yellow-500 shadow-xl overflow-hidden p-0.5">
                                            <div className="w-full h-full rounded-full overflow-hidden border border-[#003366]/5 shadow-inner">
                                                {teacher.avatar && teacher.avatar.length > 5 ? (
                                                    <img src={teacher.avatar} className="w-full h-full object-cover" crossOrigin="anonymous" />
                                                ) : (
                                                    <div className="w-full h-full flex items-center justify-center text-[#003366] bg-slate-50 font-black text-4xl">{teacher.name.charAt(0)}</div>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <div className="text-center w-full mb-0.5 mt-2">
                                        <h2 className="text-[12pt] font-black text-white uppercase leading-none tracking-tight">{teacher.name}</h2>
                                        <div className="mt-1.2 inline-block px-4 py-0.5 bg-yellow-400 text-[#003366] rounded-full">
                                            <p className="text-[5pt] font-black uppercase tracking-widest leading-none">OFFICIAL IDENTITY</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-1 w-full space-y-2 pt-1.5 border-t border-white/10">
                                        <div className="flex justify-between items-center text-white text-[10pt] font-bold">
                                            <span className="text-[7pt] font-black text-yellow-500 uppercase tracking-widest">STAFF ID:</span>
                                            <span>{teacher.id}</span>
                                        </div>
                                        <div className="flex justify-between items-center text-white text-[10pt] font-bold">
                                            <span className="text-[7pt] font-black text-yellow-500 uppercase tracking-widest">DESIGNATION:</span>
                                            <span className="uppercase ml-2 flex-1 text-right leading-tight">{teacher.subject || "OFFICIAL"}</span>
                                        </div>
                                        <div className="flex justify-between items-center text-white text-[10pt] font-bold">
                                            <span className="text-[7pt] font-black text-yellow-500 uppercase tracking-widest">QUAL:</span>
                                            <span className="uppercase truncate ml-2 flex-1 text-right">{teacher.qualification || "N/A"}</span>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="pt-1">
                                        <div className="space-y-2.5 bg-slate-50/50 p-2.5 rounded-[3mm] border border-slate-100 shadow-sm mt-1">
                                            ...
                                            <div className="flex items-center gap-2.5">
                                                ...
                                                <p className="text-[6.8pt] font-bold text-slate-800 leading-tight lowercase">{teacher.email || 'psscc.official@gmail.com'}</p>
                                            </div>
                                    ...
                                    <div className="mt-auto h-[22mm] flex items-center justify-between border-t border-slate-100 pt-2 pb-1 px-1">
                                        <div className="flex flex-col items-center">
                                            <div className="p-1 bg-white border border-slate-200 rounded-lg flex items-center justify-center shrink-0 shadow-md" style={{ width: '46px', height: '46px' }}>
                                                <QRCodeSVG value={teacher.id || "STAFF-001"} size={38} level="M" includeMargin={false} />
                                            </div>
